...
public class CartService {
	...
	/*public synchronized void addnewProduct(Cart cartData, Long productId) {
		if (cartData.contains(productId)) {
			cartData.incrementProductQuantity(productId);
		} else {
			OrderItem orderItem = cartData.getProduct(productId);
	...
	public synchronized int getNumberOfItems(CartData cartData) {
		System.out.println("cart service 0000000");
	...
	public CartData getShoppingCartByCustomer(Customer customer) {
		...
		return cartRepository.getCartDataByCustomer(customer.getCustomerId());
	...
	/*		public Cart getNewShoppingCart(Long productId,Long customerId) {
	
		Session session = sessionFactory.openSession();
		Cart cartData = (Cart) session.createQuery("from com.spring.shopping.model.Cart c where c.productId=:productId AND c.customerId=customerId").setParameter("productId", productId).setParameter("customerId", customerId).uniqueResult();
		if (cartData != null)
			return cartData;
		else
			return new Cart();
	...
	// Add Products to Shopping Cart
		/*public synchronized void addnewProduct(CartData cartData, Long productId) {
			if (cartData.contains(productId)) {
				cartData.incrementProductQuantity(productId);
			} else {
				OrderItem orderItem = cartData.getProduct(productId);
